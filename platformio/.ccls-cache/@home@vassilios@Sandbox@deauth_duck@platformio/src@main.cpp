  ...
  while (true) {
    ...
    while (ii < SCAN_ROUNDS)
    {
      ...
      for (int i = 0; i < size_charray; i++)
      {
        ...
        wifi_ap_record_t *net_lst = esp_wifi_scan_get_ap_records(MAX_LIST_SIZE, wifi_ap_record_t);
        int num_networks = array_size(net_lst);

        if (num_networks > 0)
        {
          Serial.println("Networks found: " + String(num_networks));
          #ifndef scan_result
            wifi_ap_record_t scan_result[num_networks] = net_lst;
          #endif
